...
\end{itemize}
We have $O(|V|^2)$ relabel operation, $O(|V|\cdot |E|)$ saturing push and $O(|V|^2\cdot |E|)$ non-saturing push. Thus, the generic preflow algorithm runs in $O(|V|^2) + O(|V|\cdot |E|) + O(|V|^2\cdot |E|) = O(|V|^2 \cdot |E|)$. 
