...
On the other hand, Edmonds-Karp uses a breadth-first search to look for the shortest path (in terms of number of edges) \cite{lectu7}. A bigger quantity of flow is thus sent on a smaller number of edges. \newline
...
The ping-pong effect is a phenomen that occurs in preflow-push algorithm that considerably slow it down. To describe the ping-pong effect \cite{pingpongeffect}, we will execute the preflow-push algorithm on the network in Figure \ref{img:pingpong1}.
