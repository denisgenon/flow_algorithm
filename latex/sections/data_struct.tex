...
Here is a example of sparse set : 
\begin{figure}[H]
\begin{center}
...
\end{figure}
We cannot use this structure because the neighbourhood of a vertex don't form a suite of index. For example, if a vertex had three neighbours 8, 72 and 13, we can't directly know which vertex is contains in which index.
So we created a first data structure, the split array, which is a simplified version of the sparse set without \textit{map}. We also created a second structure, the sparse map, where the array \textit{map} is replaced by a hash map.
\newpage
...
A sparse map is composed by the \textit{dom} array and a hash map containing the position of each vertex in the array. The key is the position and the value is the vertex. \newline

To add or remove a vertex, it is like for the split array but we don't need to go through the \textit{dom} array because we can obtain the position of a vertex in \textit{dom} thanks to the hash map, saving valuable time. In compensation, we need to update the hash map so it remains consistent. 
\newpage
\subsection{Complexities}

This table below represents the complexities with the notation big O, which means "in the worst case". Although for the tree map, the simple linked list and the split array, the worst case is not too penalizing but that is not the case for the hash map and thus for the sparse map. Indeed, we obtain $O(n)$ if every elements of the hash map are in the same bucket. However, with a correct hash function, we can hope $O(1)$. \newline
\begin{figure}[H]
\centering
