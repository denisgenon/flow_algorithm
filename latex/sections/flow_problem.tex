...
\end{definition}
See that there can be a path from $i$ to $j$ in the residual network, even though there is not path from $j$ to $i$ in the original network. This is used as a backtrack mechanism.
...
The solution of this problem is to find the minimum cut of the network. As we know with the Max-flow min-cut theorem \ref{mincut_maxflow}, the minimum cut is equals to the maximum flow. The proof of this theorem is outside the scope of this master thesis.
